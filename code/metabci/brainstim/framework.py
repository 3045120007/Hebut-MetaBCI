...
class Experiment:
    ...
    ----------
        ...
        翻译：
        win_size: tuple、形状(宽,高)
           以像素为单位的窗口大小[x, y]。
        screen_id: int
            屏幕的id。指定将出现刺激的物理屏幕;
            默认值为0，如果存在多个屏幕，该值可以>0。
        is_fullscr: bool
            是否在“全屏”模式下创建窗口。
            监控:监控
            在实验过程中使用的监视器，如果没有，将使用默认的监视器配置文件。
        bg_color_warm: ndarray、形状(红、绿、蓝)
            开始屏幕的背景色，格式为[r, g, b]，值在-1.0到1.0之间。
        record_frames: bool
            记录每帧经过的时间，提供帧间隔的精确测量
            确定帧是否被丢弃。
        disable_gc: bool
            禁用垃圾收集器。
        process_priority: str
